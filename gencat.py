...
s = {}
for sed in sed_list: 
	s[sed] = np.loadtxt(sed_folder + sed, unpack = True)

#Set same x for all seds...............
x_sed = s[sed_list[1]][0]
x_sed_max = 20000 
x_sed = x_sed[x_sed < x_sed_max]
s2 = {}
for sed in sed_list: 
	s2[sed] = np.zeros((2,len(x_sed)))
	s2[sed][1] = sp.interpolate.interp1d(s[sed][0], s[sed][1])(x_sed)
	s2[sed][0] = x_sed
s = s2
...
if(a == False):
	...
	for n in range(n_gal):
		...
		#Computing in_s................
		#Seds must have the same x
		in_s = np.zeros(n_filt + 1)
		y_s = coef_high * s[s_high][1] + coef_low * s[s_low][1]
		for i in range(n_filt + 1):
			filt = filt_list[i]
			in_s[i] = (sp.interpolate.interp1d(x_sed * (1 + z[n]), y_s)(x[filt]) * y_r[filt] * x[filt]).sum() * dx
		...
		m = m_ref[n] + 2.5 * (log10(in_s0) + np.log10(in_r) - log10(in_r0) - np.log10(in_s)) 
		#m = m_ref[n] + 2.5 * (np.log10(in_r) - log10(in_r0)) 
		m = " ".join(np.char.mod('%8.8f', m))
		f_noiseless.write("%s %4.4f %2.2f\n" % (m, z[n], t[n]))
...
#Compress..............................
os.system('gzip ' + f_noiseless_file)
os.system('gzip ' + f_noisy_file)
